...
def create_material(ctx:Context, src_material:materialdefinition) -> str:
    ...
    src_sprite = ctx.parser.simplespritedefs[src_material.simplespriteinst.simplespritetag]
    if src_sprite is None:
        return f"simplesprite {src_material.simplespriteinst.simplespritetag} not found"
    if len(src_sprite.frames) == 0:
        return f"simplesprite {src_material.simplespriteinst.simplespritetag} has no frames"
    if len(src_sprite.frames) > 1:
        return f"simplesprite {src_material.simplespriteinst.simplespritetag} has multiple frames, only 1 supported for now"
    frame = src_sprite.frames[0]
    if len(frame.files) == 0:
        return f"simplesprite {src_material.simplespriteinst.simplespritetag} has no files"
    if len(frame.files) > 1:
        return f"simplesprite {src_material.simplespriteinst.simplespritetag} has multiple files, only 1 supported for now"
