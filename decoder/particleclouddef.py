...
def decode_particleclouddef(ctx: Context, cloud: particleclouddef) -> str:
    ...
    emitter_mesh = bpy.data.meshes.new(
        cloud.tag
    )
    ...
    props.boundingbox = bool(
        cloud.boundingbox
    )

    props.updatebbox = bool(
        cloud.updatebbox
    )

    props.pointgravity = bool(
        cloud.pointgravity
    )

    props.gravityflag = bool(
        cloud.gravityflag
    )
    ...
    props.objectrelative = bool(
        cloud.objectrelative
    )

    props.parentobjrelative = bool(
        cloud.parentobjrelative
    )

    props.spawnscalerelative = bool(
        cloud.spawnscalerelative
    )

    props.hidewithspawnobject = bool(
        cloud.hidewithspawnobject
    )
    ...
    if cloud.blittag:
        ...
        source_blit = bpy.data.objects.get(
            cloud.blittag
        )
