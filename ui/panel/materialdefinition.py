...
from ...convert.material import convert_to_materialdefinition
...
class MATERIAL_OT_add_default_wldmatdef(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        err = convert_to_materialdefinition(material, context)

        if err:
            self.report({'ERROR'}, err)
            return {'CANCELLED'}

        self.report({'INFO'}, f"Converted {material.name} to MATERIALDEFINITION")
        return {'FINISHED'}
