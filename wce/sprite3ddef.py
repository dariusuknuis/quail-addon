...
class sprite3ddef:
	...
	def write(self, w:io.TextIOWrapper)->str:
		...
		for bspnodei in self.bspnodes:
			...
			w.write(f"\t\t\t\tBRIGHTNESS? {('NULL' if bspnodei.bspnode.renderinfo.brightness is None else format(bspnodei.bspnode.renderinfo.brightness, '.8e'))}\n")
			w.write(f"\t\t\t\tSCALEDAMBIENT? {('NULL' if bspnodei.bspnode.renderinfo.scaledambient is None else format(bspnodei.bspnode.renderinfo.scaledambient, '.8e'))}\n")
			w.write(f"\t\t\t\tSIMPLESPRITEINST\n")
			if bspnodei.bspnode.renderinfo.simplespriteinst.simplespritetag is None: w.write("\t\t\t\t\tSIMPLESPRITETAG? NULL\n")
		...
		w.write(f"\t\tSCALEFACTOR? {('NULL' if self.spherelist.scalefactor is None else format(self.spherelist.scalefactor, '.8e'))}\n")
